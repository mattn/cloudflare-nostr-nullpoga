...
function createNoteWithTags(env: Env, mention: Event, message: string, tags: string[][]): Event {
    const decoded = nip19.decode(env.NULLPOGA_NSEC)
    const sk = decoded.data as string
    const pk = getPublicKey(sk)
    const tt = []
...
async function doClock(_request: Request, env: Env): Promise<Response> {
    ...
    const mention = { kind: 1, tags: [], created_at: Math.floor(Date.now() / 1000) }
    return new Response(JSON.stringify(createNoteWithTags(env, mention, message, [])), {
        headers: {
            'content-type': 'application/json; charset=UTF-8',
        },
    })
...
const bookmarks: bookmark[] = [
    ...
    { pattern: /^(nosli|のすり|かまくらさんのアレ|鎌倉さんのアレ|togetterみたいなやつ|togetterみたいな奴)$/i, site: 'https://nosli.vercel.app/' },
    ...
    { pattern: /^(野須田|流速|観測所|野須田川観測所|野洲田川定点観測所)$/i, site: 'https://nostr-hotter-site.vercel.app/' },
...
async function doGrave(request: Request, env: Env): Promise<Response> {
    ...
    let arr = mention.content.replace(/の墓$/, '').split(/(:[^:]+:)/g).map((x: string) => {
        if (/^(:[^:]+:)$/.test(x)) return [x]
        //return [...x.replace(/[A-Za-z0-9]/g, (s) => String.fromCharCode(s.charCodeAt(0) + 0xFEE0)).replace(/[ー〜]/g, '｜')]
        return runes(x.replace(/[A-Za-z0-9]/g, (s) => String.fromCharCode(s.charCodeAt(0) + 0xFEE0)).replace(/[ー〜]/g, '｜'))
    }).flat()
...
async function doTranslate(request: Request, env: Env): Promise<Response> {
    ...
    const tags = mention.tags.filter((x: any[]) => x[0] === 'emoji')
    return new Response(JSON.stringify(createReplyWithTags(env, mention, response.translated_text, tags)), {
        headers: {
            'content-type': 'application/json; charset=UTF-8',
        },
    })
...
async function doMetadata(request: Request, env: Env): Promise<Response> {
    ...
    const content = `${profile['display_name'].trim()} さんがプロフィールを更新しました`
    metadata.kind = 1
    return new Response(JSON.stringify(createNoteWithTags(env, metadata, content, tags)), {
        headers: {
            'content-type': 'application/json; charset=UTF-8',
        },
    })
...
async function doSleeply(request: Request, env: Env): Promise<Response> {
    const mention: Event = await request.json()
    return new Response(JSON.stringify(createReplyWithTags(env, mention, "(`･д･⊂彡☆))Д´)) ﾊﾟｧﾝ", [])), {
        headers: {
            'content-type': 'application/json; charset=UTF-8',
        },
    })
...
async function doHit(request: Request, env: Env): Promise<Response> {
    const mention: Event = await request.json()
    return new Response(JSON.stringify(createNoteWithTags(env, mention, "(`･д･⊂彡☆))Д´)) ﾊﾟｧﾝ", [])), {
        headers: {
            'content-type': 'application/json; charset=UTF-8',
        },
    })
